...
class/struct 子类名称：继承方式 基类名称
	...
	2. 基类成员在子类的访问权限
		...
		b. 基类其他成员在子类中的访问权限为min{成员在基类中本身的访问权限，子类的继承方式}取最小
	3. protected，public
		a. 都表示在类外不可见
...
子类赋给父类：切片（不属于隐式类型转化———）
父类的对象不能赋给子类，
父类当中的内容没有包括子类，可能会导致出错，安全风险，父类指针转换子类指针是有一种安全转换机制
...
同名隐藏： 继承关系中，父类和子类有名字相同的的成员（成员变量、成员函数）
	直接在子类访问到的是子类自己的成员函数，
	如果需要访问父类同名的成员，需要加父类作用域，（父类：：父类函数名）
同名隐藏包含函数隐藏

子类构造： 默认和显式定义的逻辑相同，编译器会自动去调用父类的构造函数，也可以在初始化列表中进行改变，在子类的初始化列表自动调用父类的构造，先完成父类的构造，再完成子类的
子类拷贝构造： 默认调用父类拷贝构造，显式定义之后，如果不显式调用拷贝构造时，编译器自动调用父类的默认构造，我们把子类的对象传给父类的构造发生切片，把父类那一部分切出来 再去拷贝构造继承父类的成员
子类的赋值运算符： 默认调用父类的赋值运算符。显式定义之后，子类的赋值运算符和父类的赋值构成同名隐藏，不会自动调用父类的赋值运算符， 除非显式指定调用，子类和父类的赋值运算符构成同名隐藏
子类的析构： 默认和显式定义逻辑相同，编译器默认给子类生成的析构会自动调用父类的析构，先完成自己的清理再完成父类的析构，

继承关系下的构造和析构顺序：
	创建子类对象： 先调用子类构造函数，在子类构造函数的初始化列表中调用父类构造函数，首先完成父类的构造逻辑，再执行子类的构造逻辑
	销毁子类对象： 先调用子类析构，首先执行完子类的析构逻辑，再去调用父类的析构函数，完成父类的析构逻辑
友员关系不继承
静态成员只有一个，那个需要访问，不存在对象中，存在静态数据段
